(function () {
  ...
  function getValidImagePosition(value) {
    if (typeof value !== "string") return "";

    var match = value
      .trim()
      .match(
        /^((?:0|[1-9]\d?|100)(?:\.\d+)?)% ((?:0|[1-9]\d?|100)(?:\.\d+)?)%$/,
      );
    if (!match) return "";

    var horizontal = Number(match[1]);
    var vertical = Number(match[2]);
    if (
      !isFinite(horizontal) ||
      !isFinite(vertical) ||
      horizontal < 0 ||
  ...
  function setImage(imageEl, image, imageAlt, imagePosition) {
    if (!imageEl) return;

    var validImagePosition = getValidImagePosition(imagePosition);
    if (validImagePosition) {
      imageEl.style.objectPosition = validImagePosition;
    } else {
  ...
  function hasEventImage(event) {
    return Boolean(
      event && typeof event.image === "string" && event.image.trim()
    );
  }

  function togglePhotoText() {
    if (!state.events.length) return;
    if (!hasEventImage(state.events[0])) return;
  ...
  function renderView(els) {
    ...
    setImage(els.imageEl, image, imageAlt, event.imagePosition);

    fillEventFields(els, event);

    if (!image) {
      if (els.questionEl) els.questionEl.textContent = "";
      if (els.coverRowEl) els.coverRowEl.setAttribute("hidden", "");
      if (els.answerEl) els.answerEl.hidden = false;
      if (els.imageEl) els.imageEl.hidden = true;
      ...
      els.card.classList.add("card--history-plain");
    ...
    els.card.classList.remove("card--history-empty", "card--history-plain");
